...
estado -->
                    ...
                    <table class="table table-bordered">
                      ...
                      <tbody>
                       <?php
						  $sqlvista="SELECT * FROM usuarios INNER JOIN niveles 
                         ON usuarios.idnivel = niveles.idNivel WHERE estado = 1 ORDER BY 1 DESC";
...
// idusuario, nombre, apellidos, dni, correo, celular, usuario, password, idnivel, acceso, fecha_reg, estado
	  ...
	  $id = $rwpv["idusuario"];
	  $nombre = $rwpv["nombre"];
	  $apellidos = $rwpv["apellidos"];
	  $dni = $rwpv["dni"];
	  $correo = $rwpv["correo"];
	  $celular = $rwpv["celular"];
	  $usuario = $rwpv["usuario"];
	...
	echo"<td>$usuario</td>";
	  ...
	  $btnaxe
	  <a href='delete-admin.php?dat=$id' onClick='return confirmar2()'>
