                    ...
                    <table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
                      <thead>
                        <tr>
                          ...
                          <th class="th-sm">Especialidad <i class="fa fa-sort float-right" aria-hidden="true"></i></th>
...
// idespecialidad, especialidad, descrip_especialida, nom_corto_esp, estado_esp
        						  ...
        						  $sqlvista="SELECT * FROM especialidades WHERE estado_esp=1 ORDER BY 1 DESC";
        						  $rspv=$linkdocu->query($sqlvista);
                      if($rspv->num_rows>0){
                        while($rwpv=$rspv->fetch_array()){
                      	  $cca = $cca + 1;
                          $id = $rwpv["idespecialidad"];
                          $especialidad = $rwpv["especialidad"];
                          $desc_espec = $rwpv["descrip_especialida"];
                          $nomcortoesp = $rwpv["nom_corto_esp"];
                      ?>    

            <tr>
              <td class='font-weight-medium'><?php echo $cca; ?></td>
              <td><?php echo $especialidad; ?></td>
              <td><?php echo $desc_espec; ?></td>
              <td>
                <a href='espec_edit.php?dat=<?php echo $id;?>' class='btn btn-icons btn-rounded btn-success' title='Editar'><i class='mdi mdi-pencil'></i></a>
                <a href='delete_espec.php?dat=<?php echo $id;?>' onClick='return confirmar2()'>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
         <p style="color: #E10D11;font-size: 12px;"><b>Campos requeridos (*)</b></p>
          <form class="forms-sample" method="post" action="insert_espec.php" enctype="multipart/form-data">
            <div class="row">
              <div class="col-md-12">
                      
                <div class="form-group">
                  <label for="especialidad">Especialidad <b style="color: #DD070B;font-size: 12px;">(*)</b></label>
                  <input type="text" name="especialidad" class="form-control" placeholder="Ingrese especialidad" required >
                </div>

                <div class="form-group">
                  <label for="descespec">Descripción <b style="color: #DD070B;font-size: 12px;">(*)</b></label>
                  <input type="text" name="descespec" class="form-control" placeholder="Ingrese descripción escpecialidad" required >
                </div>                      

                <div class="form-group">
                  <label for="nomcespec">Nombre Corto <b style="color: #DD070B;font-size: 12px;">(*)</b></label>
                  <input type="text" name="nomcortoespec" class="form-control" placeholder="Ingrese nombre corto" required >
                </div>  
